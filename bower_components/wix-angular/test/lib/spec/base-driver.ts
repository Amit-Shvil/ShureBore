...
class WixBaseViewUnitDriver {
  ...
  private parent: WixBaseViewUnitDriver;
  private templateRoot: ng.IAugmentedJQuery;
  private childDrivers: Array<ChildDriver> = [];
  ...
  protected initChildDrivers() {
    this.childDrivers.forEach(child => {
      if (child.type === ChildDriverType.CHILD_REGULAR) {
        this.initRegularChild(child);
      } else if (child.type === ChildDriverType.CHILD_ARRAY) {
  ...
  protected defineChildren<T extends WixBaseViewUnitDriver>(factory: (item?, index?) => T, selector: string): Array<T> {
    ...
    this.childDrivers.push({
      type: ChildDriverType.CHILD_ARRAY,
      selector,
      factory,
      drivers: children,
      fullDriversArr: []
  ...
  private defineIndexedChild<T extends WixBaseViewUnitDriver>(childDriver: T, selector?: string, selectorIndex: number = 0): T {
    this.childDrivers.push({
      selector,
      selectorIndex,
      type: ChildDriverType.CHILD_REGULAR,
      drivers: [childDriver]
    });
